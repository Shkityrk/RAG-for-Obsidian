import { getChatMessages, sendChatMessage, cleanChatMessages, sendDeepResearchMessage } from "../api/messages";
...
const useSendDeepResearchMessage = () => {
    const queryClient = useQueryClient();
    return useMutation(
        {
            mutationKey: ["send_deep_research_message"],
            mutationFn: (data: QueryRequest) => sendDeepResearchMessage(data),
            onError: handleAPIError,
            onSuccess: (_, variables) => {
                queryClient.invalidateQueries({ queryKey: ["chat_messages", variables.chat_id] });
                queryClient.invalidateQueries({ queryKey: ["llm_tokens"] });
...
export {useChatMessages, useSendChatMessage, useCleanChatMessage, useSendDeepResearchMessage};
